...
function backFillLandscapePhotos(photoSet: PhotoSetObject[], portraitCount: number): PhotoSetObject[] {
...
function sortImages(photoSet: PhotoSetObject[]) {
	...
	for (let i=0; i<photoSet.length; i++) {
		const isLandscape: boolean = photoSet[i].data.landscape_portrait == 'landscape' ? true : false
...
export default function PhotoSet({ photoSet }: PhotoSetProps) {
	...
	const setImages = photoSet.map((imageObject: any, index: number) => {
		...
		const photoWidth = data.landscape_portrait == 'landscape' ? styles.landscapePhoto : styles.portraitPhoto
		return (
			<li key={index} className={photoWidth}>
				<Image src={data.filename_disk} 
	...
	const setHeader = header ? <Image src={header.data.filename_disk} loader={({src}) => `https://api.shanelhonolulu.com/assets/${src}`} 
	alt={`set header`} layout='fill' objectFit='cover' /> : undefined
